...
@push('scripts')
    <script type="text/javascript" src="{{ mix('js/family.cash-flow-plans.income-sources._form.js') }}"></script>
@endpush
...
<form name="incomeSource" action="{{ $action }}" method="POST" class="has-bold-labels">
    ...
    <fieldset>
        ...
        <div class="form-group">
            <label for="actual">{{ __('income-sources.actual') }}</label><button type="button" class="copy-value btn btn-sm btn-link" data-copy-source="#projected" data-copy-target="#actual">{{ __('cash-flow-plans.copy-from-projected') }}</button>
            <div class="input-group">
                <div class="input-group-prepend">
                    <div class="input-group-text"><span class="fa fa-dollar"></span></div>
                </div>
