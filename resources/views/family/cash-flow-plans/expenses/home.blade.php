                ...
                <table class="table table-sm">
                    ...
                    @foreach ($expenses->where('category_id', null) as $expense)
                        ...
                        <tr id="expense_{{ $expense->id }}" data-expense-id="{{ $expense->id }}">
                            <td style="border-left: 4px solid transparent" title="{{ $expense->merchant->name }} - {{ __('expenses.uncategorized') }}">{{ Auth::user()->formatDate($expense->date) }}</td>
                            <td title="{{ $expense->merchant->name }} - {{ __('expenses.uncategorized') }}"><a href="{{ route('family.cash-flow-plans.expenses.edit', [$family, $cashFlowPlan, $expense]) }}">{{ $expense->merchant->name }}</a></td>
                    ...
                    @foreach ($categories as $category)
                        @foreach ($expenses->where('category_id', $category->id) as $expense)
                            <tr id="expense_{{ $expense->id }}" data-expense-id="{{ $expense->id }}">
                                <td style="border-left: 4px solid {{ $category->color }}" title="{{ $expense->merchant->name }} - {{ $category->name }}">{{ Auth::user()->formatDate($expense->date) }}</td>
                                <td title="{{ $expense->merchant->name }} - {{ $category->name }}"><a href="{{ route('family.cash-flow-plans.expenses.edit', [$family, $cashFlowPlan, $expense]) }}">{{ $expense->merchant->name }}</a></td>
                                <td class="text-right">{{ Auth::user()->formatCurrency($expense->projected, true) }}</td>
                                <td class="text-right">{{ Auth::user()->formatCurrency($expense->actual, true) }}</td>
                            </tr>
                        @endforeach
